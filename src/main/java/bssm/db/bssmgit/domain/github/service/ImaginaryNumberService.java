...
import static bssm.db.bssmgit.domain.github.domain.type.Imaginary.IMAGINARY_NUMBER;
...
public class ImaginaryNumberService {
    ...
    public void updateImaginaryNumberUser() {
        ...
        for (Long userId : userIds) {
            long count = userIds.stream()
                    ...
                    .count();
            if (count > 4) {
                GitHub gitHub = userFacade.findById(userId).getGitHub();
                gitHub.updateImaginary();
            }
        }
    ...
    private void initImaginaryReportsLessThan5() {
        ...
        for (Long userId : userIds) {
            if (reportsLessThan5(userIds, userId)) {
                GitHub gitHub = userFacade.findById(userId).getGitHub();
                if (gitHub.getImaginary() == IMAGINARY_NUMBER) {
                    gitHub.initImaginary();
                }
            }
        }
        userFacade.saveAll(users);
    ...
    private boolean notExistsRecordReportButAdaptImaginaryNumberUser() {
        Long currentUserId = userFacade.getCurrentUser().getId();

        return imaginaryNumberFacade.findAll()
                .stream()
                .anyMatch(imaginaryNumber -> Objects.equals(imaginaryNumber.getReportedUserId(), currentUserId));
    }
}
